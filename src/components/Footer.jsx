const socialLinks = [
  {
    href: "https://github.com/saroj1237",
    label: "GitHub",
    icon: (
  ...
  {
    href: "https://www.linkedin.com/in/saroj-basnet-54a50420b/",
    label: "LinkedIn",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <footer className="border-t border-border bg-surface mt-auto">
      <div className="section-container py-8 flex flex-col items-center gap-4">
        <div className="flex items-center gap-4">
          {socialLinks.map(({ href, label, icon }) => (
            <a
              ...
              rel="noreferrer"
              aria-label={label}
              className="p-2 rounded-lg text-muted transition-colors duration-200 hover:text-accent hover:bg-bg"
            >
              {icon}
            </a>
          ))}
        ...
        <p className="text-sm text-muted">
          © {new Date().getFullYear()} Saroj Basnet. All rights reserved.
        </p>
      </div>
    </footer>
  );
